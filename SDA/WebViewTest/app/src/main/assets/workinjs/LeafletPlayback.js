...
L.Playback.PlayControl = L.Control.extend({
    ...
    onAdd : function (map) {
        ...
        this._button.innerHTML = 'Запустить';
        ...
        function play(){
            if (playback.isPlaying()) {
                playback.stop();
                self._button.innerHTML = 'Запустить';
            }
            else {
                playback.start();
                self._button.innerHTML = 'Остановить';
            }                
        }
...
L.Playback = L.Playback.Clock.extend({
        ...
        options : {
            tickLen: 25000,
            speed: 10,
            maxInterpolationTime: 1*60*1000000, // 1 minutes
